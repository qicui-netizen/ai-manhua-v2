...
9. 【特征锁定】charactersInPanel[].lockedTraits 表示该角色各特征相对参考图的锁定强度
   ...
   - 不锁定：完全交给画面需要，不做约束。
   negativeTraits 是用户手写的禁改要求（中文），逐条转成简短英文并入 negativePrompt。
...
export function offlineImageEditPrompt(input: ImageEditPromptInput): ImageEditPromptResult {
  ...
  const userNegatives = input.characters.flatMap((c) => c.negativeTraits ?? []);
  return {
    panelId: String(input.panel.panelId),
    blocked: false,
    imageSlots: slots,
    // 构图安全区后缀与规则10同理:成图会按目标比例居中裁剪,主体必须收在中央。
    // 用户修正要求在 slice 截断之后单独拼接,保证兜底路径下修正不丢失
    editPrompt:
    ...
    negativePrompt: [...new Set(["多余肢体、文字错乱、风格混杂、水印", ...lockNegatives, ...userNegatives])].join(", "),
    aspectRatio: input.aspectRatio,
