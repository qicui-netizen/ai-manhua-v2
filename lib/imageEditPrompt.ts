...
【输入】
{
  ...
  "panel": {
    ...
    "unmatchedCharactersInPanel": [
      {
        "name": "string，剧情里出现但用户未建角色卡、没有参考图的人物称呼",
        "appearanceAnchor": "string，分镜阶段为该人物生成的固定外貌描述，全篇每格必须使用完全相同的这句话"
      }
    ],
...
10.【构图安全区】出图模型不支持指定输出尺寸，成图比例会跟随参考图，最终按
   ...
   同时追加一句合理性约束,防止模型把人物异常淹没/遮挡(如站在积水里水面盖到胸口、被前景物挡住半身):
   "characters standing naturally on solid ground with full upper body visible, not submerged or
   waist-deep in water, not blocked by foreground objects"，除非 visualPromptHint 明确要求人物在水中。
11.【风格锚·全篇一致铁律】style.anchor 是本篇统一画风的权威英文原文。你必须把 style.anchor
   ...
   另外——无论 anchor 是否为空——negativePrompt 都必须固定加入去水印/去杂物负向词：
   "watermark, logo, text, caption, signature, watermark text, brand logo, qr code, frame, border"，
   因为参考图常自带平台水印/logo，图生图会把它们复刻进成图，必须用负向词压制。
...
14.【未建卡角色·文字外貌锚】unmatchedCharactersInPanel 里的人物没有参考图、不占图片槽位,完全依赖文字
   描述让模型"画出"这个人。对每一个出现在本格的这类人物:
   (a) 必须把其 appearanceAnchor 翻译成英文,原样（不改写、不精简、不换措辞）写入 editPrompt 的画面
       描述部分,作为这个人物的外观依据，如 "a girl with shoulder-length straight black hair, round
       face, wearing a white dress"。
   (b) 同一个人物在全篇不同格出现时使用的英文措辞必须逐字相同(不要在不同格里用不同的同义词改写),
       这是保证TA在没有参考图的情况下,仍能在多格图里长相基本一致的唯一手段。
   (c) 若该人物同时与 charactersInPanel 里的有卡角色同框,同样适用规则13的"区分不同角色"要求,
       不要让TA和有卡角色的长相混同。
   unmatchedCharactersInPanel 为空数组时跳过本规则。
15.【身体比例铁律·治大头小身】图生图模型有把人物画成"大头小身"（类似Q版比例）的默认倾向，即使
   风格锚里写了"natural proportions"这类软性描述也常压不住这个倾向，必须用更强的显式约束纠正：
   (a) 如果 style.label **不是**"Q版"（即该篇画风不是刻意要大头萌系比例），必须在 editPrompt 里
       追加一句强制成人比例的英文约束，紧跟在构图安全区约束（规则10）之后：
       "realistic adult human body proportions, natural 7 to 7.5 heads tall figure, head size
       proportional to body, NOT chibi, NOT super-deformed, NOT oversized head"。
       同时在 negativePrompt 里固定加入："big head small body, disproportionate head, oversized
       head, chibi proportions, super-deformed, baby-like body proportions"。
   (b) 如果 style.label **是**"Q版"，本规则不生效——Q版本身就是刻意的大头萌系比例（2头身），
       不要加上述成人比例约束和负向词，那样会与 style.anchor 里的 chibi 比例要求自相矛盾。
16.【视觉基准锚·治全篇画风漂移】feedbackRefs.baseAnchorSlot 若为非0整数，表示图{baseAnchorSlot}
   是本篇【第1格已生成的成图】，是全篇画风、色调、渲染质感的权威视觉基准（比文字风格锚更强，因为它
   是"示范"而非"描述"）。你必须在 editPrompt 里追加一句英文，位置在构图安全区/身体比例约束（规则10、15）
   之后、style.anchor（规则11）之前：
   "match the overall art style, color grading and rendering quality of 图{baseAnchorSlot} as the
   series visual reference"。
   【严格边界·必须遵守，否则会毁掉本格】只借图{baseAnchorSlot}的【画风/色调/渲染】三样，绝不照搬：
   (a) 不要照搬它的构图与人物站位——本格构图仍严格以 panel.visualPromptHint 为准；
   (b) 不要用它里面的人脸覆盖本格角色——角色身份仍以各角色参考图（图1/图2…）为准；
   (c) 不要把它当作"要编辑的底图"——它只是风格样板，本格是重新绘制的新画面。
   feedbackRefs.baseAnchorSlot 为0或缺失时（如第1格，或未启用回喂）跳过本规则，不追加任何内容。
...
export type ImageEditPromptInput = {
  ...
  // 剧情提到但用户未建卡、没有参考图的人物(如凭空出现的女主),按规则14用固定外貌锚描述引用
  ...
  adjustHint?: string;
  // P0-2 视觉基准锚:第1格成图作为全篇画风基准回喂给后续格时,它在参考图数组里的编号(图N)。
  // route 层把角色图放前、基准图拼在其后,故 baseAnchorSlot = 角色图数 + 1。为0或缺省表示本格
  // 不回喂(如第1格自身、或未启用),生图指令 Agent 按规则16跳过。route 需保证与实际图片顺序一致。
  feedbackBaseSlot?: number;
};
...
function buildUserPayload(input: ImageEditPromptInput) {
  return {
    ...
    panel: {
      ...
      // 只传本格 character_action 里实际点名出场的野生角色,和多角色同框的角色过滤口径一致
      unmatchedCharactersInPanel: (input.unmatchedCharacters ?? []).filter((u) => input.panel.characterAction.includes(u.name)),
    ...
    // P0-2 视觉基准锚:非0表示图{baseAnchorSlot}是第1格成图回喂,生图指令 Agent 按规则16借其画风
    feedbackRefs: { baseAnchorSlot: input.feedbackBaseSlot ?? 0 },
  };
}
...
export function offlineImageEditPrompt(input: ImageEditPromptInput): ImageEditPromptResult {
  ...
  const styleNegatives = input.styleAnchor
    ? ["inconsistent art style", "mixed art styles", "style drift", "different art style", "off-model"]
    : [];
  // 野生角色(剧情提到但未建卡)的固定外貌锚:离线路径同样拼入,只传本格实际出场的,
  // 保持与 LLM 路径(规则14)一致的口径,防止降级路径下这个人物的外观描述整段丢失
  const unmatchedInPanel = (input.unmatchedCharacters ?? []).filter((u) => input.panel.characterAction.includes(u.name));
  const unmatchedTail = unmatchedInPanel.length ? `, ${unmatchedInPanel.map((u) => u.appearanceAnchor).join(", ")}` : "";
  return {
    panelId: String(input.panel.panelId),
    blocked: false,
    imageSlots: slots,
    ...
    negativePrompt: [
      ...new Set([
        "多余肢体、文字错乱、风格混杂",
        // 去水印/去杂物:参考图常自带平台水印,图生图会复刻进成图,必须英文负向词压制
        "watermark, logo, text, caption, signature, brand logo, qr code, frame, border",
        ...lockNegatives,
        ...styleNegatives,
      ]),
    ].join(", "),
